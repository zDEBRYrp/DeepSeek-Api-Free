...
> Референс `vendor/notion2api` (в git не входит) уже разобран. Что взято:
> пул профилей с cooldown, Bearer-авторизация, per-IP лимит, обогащённый
> `/healthz`, реестр режимов + чек-лист добавления, суммаризатор (у нас —
> через любой OpenAI-совместимый endpoint, а не один вендор), login-CLI,
> healthcheck/ротация логов/profiles-volume в compose, идея Web UI, CORS,
> favicon, access-лог. Что отклонено: slowapi (свой лимитер без зависимости),
> привязка к SiliconFlow, per-mode лимиты (нам не дают ничего), навороченный
> фронтенд с темами/анимациями (хватит минимального `/ui`), их SQLite-менеджер
> диалогов (покрыто `MEMORY_MODE` + суммаризатором), HTTP-мультиаккаунт
> (неприменимо — у нас браузер, а не HTTP API).
...
- ✅ **Мини Web UI** (`GET /ui`): чат, режимы, панель мышления, цитаты.
  ❌ Полноценный клон Notion AI Studio (темы/анимации/история диалогов) — не нужен.
