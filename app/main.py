...
def _build_tool_instruction(tools: Optional[List[Tool]]) -> str:
    ...
    if not tools:
        return ""
    import os as _os
    work_dir = settings.WORK_DIR or _os.getcwd()
    lines = [
        ...
        "ВАЖНО: если решил использовать инструмент — ОБЯЗАТЕЛЬНО выведи JSON-вызов в "
        "ЭТОМ ЖЕ сообщении. Краткое пояснение перед ним допустимо (например, "
        "'Проверю файлы:'), но сам вызов ОБЯЗАН присутствовать — НЕ заканчивай ответ "
        "одним лишь обещанием вроде 'сейчас проверю' или 'узнаю'. Вызов делай СЕЙЧАС, "
        "в этом сообщении; выведи его на отдельной строке без markdown-заборов и без "
        "восклицательных знаков рядом. Краткое пояснение перед вызовом (1 строка) "
        "допустимо, но клиент может НЕ показывать текст ПЕРЕД вызовом инструмента — "
        "поэтому основное объяснение и вывод давай уже ПОСЛЕ получения результата "
        "(в следующем ответе), там оно точно отобразится. РЕЗУЛЬТАТ работы инструмента "
        "придёт отдельным сообщением в СЛЕДУЮЩЕМ ответе, и ты продолжишь работу уже на его основе.",
